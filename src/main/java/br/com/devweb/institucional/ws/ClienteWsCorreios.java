...
import br.com.devweb.institucional.ws.model.CorreiosEndereco;
...
public class ClienteWsCorreios {
     ...
     */
    public static CorreiosEndereco getEnderecoPorCep(String cep) {

        Document document = getCepResponse(cep);
        CorreiosEndereco endereco = null;

        if (document != null) {
            Map<String, String> mapa = buscaNodes(document.getChildNodes(), new HashMap<String, String>());

            endereco = new CorreiosEndereco()
                    .setCep(mapa.get("cep"))
                    .setUf(mapa.get("uf"))
